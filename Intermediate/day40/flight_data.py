...
def find_cheapest_flight(data, return_date):
    if data is None or (
        not data.get("best_flights") and not data.get("other_flights")
    ):
    ...
    out_date = first_flight["flights"][0]["departure_airport"]["time"].split(
        " "
    )[0]
